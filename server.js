...
app.post("/api/auth/register", csrfProtection, (req, res) => {
  ...
  try {
    ...
    const info = db
      .prepare(
        "INSERT INTO users (username, email, password_hash, avatar_path, created_at) VALUES (?, ?, ?, ?, ?)"
      )
      .run(username, email, hash, "/pic/avatar-default.png", nowMs());
...
function resolveRecipientId(toUserId, toUsername) {
  const id = Number(toUserId);
  if (id && getUserById(id)) return id;
  const name = String(toUsername ?? "").trim();
  if (!name) return null;
  const row = db.prepare("SELECT id FROM users WHERE username = ? COLLATE NOCASE").get(name);
  return row ? row.id : null;
}

app.post("/api/messages/send", csrfProtection, requireAuth, (req, res) => {
  const toUserId = resolveRecipientId(req.body.toUserId, req.body.toUsername);
  const body = sanitizeText(req.body.body, 2000);
  if (!toUserId) {
    return res.status(404).json({
      error: "NOT_FOUND",
      message: "Пользователь не найден. Проверьте никнейм или ID."
    });
  }
  if (toUserId === req.session.userId) {
    return res.status(400).json({ error: "SELF", message: "Нельзя отправить сообщение самому себе." });
  }
  if (!body) return res.status(400).json({ error: "BODY_REQUIRED", message: "Введите текст сообщения." });
...
function aiError(res, e) {
  if (e.code === "AI_NOT_CONFIGURED") {
    return res.status(503).json({
      ...
      message:
        "ИИ не настроен. Создайте файл .env и укажите GIGACHAT_CLIENT_ID и GIGACHAT_SECRET_KEY (см. .env.example)."
    });
  }
...
app.listen(PORT, HOST, () => {
  ...
  if (aiService.isConfigured()) {
    ...
    console.log("ИИ: выключен — укажите GIGACHAT_CLIENT_ID и GIGACHAT_SECRET_KEY в .env");
  }
});
