...
const DEFAULT_AVATAR = "/pic/avatar-default.png";
...
function toggleMobileNav() {
  if (document.body.classList.contains("sidebar-open")) closeMobileNav();
  else openMobileNav();
}

function wrapTabsScroll(root = document) {
  $all(".tabs", root).forEach((tabs) => {
    if (tabs.closest(".tabs-scroll")) return;
...
function normalizeTopbar() {
  ...
  if (searchBox) {
    const btn = $("#searchBtn", searchBox);
    const input = $("#searchInput", searchBox);
    if (btn && input && btn !== searchBox.firstElementChild) {
      searchBox.insertBefore(btn, input);
    }
  }

  const createBtn = $('.actions > a.btn[href="/create"]');
  if (createBtn && !createBtn.querySelector(".create-label")) {
    const text = [...createBtn.childNodes]
      .filter((n) => n.nodeType === Node.TEXT_NODE)
      .map((n) => n.textContent.trim())
      .join("");
    createBtn.childNodes.forEach((n) => {
    ...
    if (text) {
      ...
      createBtn.append(span);
    }
  }

  const actions = $(".actions");
  if (actions && !actions.querySelector('a[href="/messages"]')) {
    const msg = document.createElement("a");
    msg.className = "icon-btn";
    msg.href = "/messages";
    msg.title = "Сообщения";
    msg.innerHTML = `<img class="topbar-icon" src="/pic/messages.png" alt="" />`;
    actions.insertBefore(msg, actions.firstElementChild);
  }

  const iconMap = [
    ['a[href="/messages"]', "/pic/messages.png", "Сообщения"],
    ['a[href="/notifications"]', "/pic/bell.png", "Уведомления"]
  ];
  iconMap.forEach(([sel, src, title]) => {
    const link = $(`.actions ${sel}`);
    if (!link || link.querySelector(".topbar-icon")) return;
    link.title = title;
    link.innerHTML = `<img class="topbar-icon" src="${src}" alt="" />`;
  });

  const create = $('.actions > a.btn[href="/create"]');
  if (create && !create.querySelector(".topbar-icon")) {
    const label = create.querySelector(".create-label");
    create.innerHTML = `<img class="topbar-icon" src="/pic/pencil.png" alt="" />`;
    if (label) create.append(label);
    else {
      const span = document.createElement("span");
      span.className = "create-label";
  ...
  if ($(".sidebar") && !$("#topbarMenuBtn")) {
    const menuBtn = document.createElement("button");
    menuBtn.id = "topbarMenuBtn";
    menuBtn.type = "button";
    menuBtn.className = "icon-btn topbar-menu-btn";
    menuBtn.title = "Меню";
    menuBtn.innerHTML = `<span class="icon" style="mask-image:url('/pic/menu-burger.png');-webkit-mask-image:url('/pic/menu-burger.png')" aria-hidden="true"></span>`;
    menuBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      if (isMobileNav()) toggleMobileNav();
    });
    const actions = $(".actions");
    if (actions) actions.insertBefore(menuBtn, actions.firstChild);
  }

  const sidebarHead = $(".sidebar .head");
  if (sidebarHead && !$("#sidebarClose")) {
...
async function initShell() {
  ...
  if (sidebar && toggle) {
    ...
    if (!isMobileNav() && localStorage.getItem("navCompact") === "1") {
  ...
  if (authZone) {
    ...
    if (!me.user) {
      ...
      login.href = "/login";
      login.title = "Войти";
      login.innerHTML = `<img class="topbar-icon" src="/pic/user.png" alt="" />`;
      authZone.append(login);
    } else {
      const profile = document.createElement("a");
      profile.className = "icon-btn user-avatar-btn";
      profile.href = "/profile";
      profile.title = `${me.user.username} — профиль`;
      profile.innerHTML = `<img class="header-avatar" src="${avatarUrl(me.user.avatar_path)}" alt="" />`;
      authZone.append(profile);
    }
  }
    ...
    <div class="card">
      <div class="profile-head">
        ...
        <div>
          ...
          <div class="muted">ID: <strong>${u.id}</strong> • @user${u.id}</div>
            ...
            isMe
              ...
              : me.user
                ? `<button class="btn primary" id="followBtn" type="button">${
                   ...
                   <a class="btn ghost" href="/messages?to=${u.id}&user=${encodeURIComponent(u.username)}">Сообщение</a>`
...
async function initMessages() {
  ...
  const presetUser = url.searchParams.get("user") || "";

  root.innerHTML = `
    <div class="card messages-hint">
      <div style="font-weight:800;margin-bottom:6px">Как написать сообщение</div>
      <ol>
        <li>Откройте профиль человека и нажмите <strong>«Сообщение»</strong> — получатель подставится автоматически.</li>
        <li>Или введите <strong>никнейм</strong> (например, <em>Семья</em>) либо числовой <strong>ID</strong>.</li>
      </ol>
      <div class="muted">Ваш ID: <strong>${me.user.id}</strong> (в профиле: @user${me.user.id}). ID — это номер аккаунта в базе сайта, он уникален для каждого пользователя.</div>
    </div>
    <div style="height:12px"></div>
    <div class="grid2">
      <div class="card">
      ...
      <div class="card">
        ...
        <form id="sendForm">
          <div class="field">
            <label>Кому (никнейм или ID)</label>
            <input class="input" id="toRecipient" value="${escapeHtml(presetUser || (to ? String(to) : ""))}" placeholder="Семья или 2" autocomplete="off" />
            <div class="hint">Можно указать никнейм или только цифры ID</div>
          </div>
          <div class="field">
            <label>Текст сообщения</label>
            <textarea class="input" id="msgBody" rows="5" placeholder="Напишите сообщение…"></textarea>
  ...
  if (!inboxData.messages.length) {
    ...
    inboxData.messages.forEach((m) => {
      const el = document.createElement("div");
      el.className = "post message-row";
      el.innerHTML = `
        <div class="avatar" style="width:44px;height:44px"><img alt="" src="${avatarUrl(m.from_avatar)}"></div>
        <div style="flex:1;min-width:0">
          <div class="meta">${fmtDate(m.created_at)} • ${escapeHtml(m.from_username)} (ID ${m.from_user_id})</div>
          <div style="white-space:pre-wrap">${escapeHtml(m.body)}</div>
          <button type="button" class="btn ghost reply-btn" style="margin-top:8px;padding:6px 10px;font-size:12px">Ответить</button>
        </div>
      `;
      el.querySelector(".reply-btn").addEventListener("click", () => {
        $("#toRecipient").value = m.from_username;
        $("#msgBody").focus();
      });
      inbox.append(el);
    });
  ...
  $("#sendForm").addEventListener("submit", async (e) => {
    e.preventDefault();
    $("#msgErr").textContent = "";
    const recipient = $("#toRecipient").value.trim();
    const body = $("#msgBody").value.trim();
    if (!recipient) {
      $("#msgErr").textContent = "Укажите никнейм или ID получателя.";
      return;
    }
    if (!body) {
      $("#msgErr").textContent = "Введите текст сообщения.";
      return;
    }
    const payload = /^\d+$/.test(recipient)
      ? { toUserId: Number(recipient), body }
      : { toUsername: recipient, body };
    try {
      await api("/api/messages/send", { method: "POST", body: payload });
      $("#msgBody").value = "";
      location.href = "/messages";
    } catch (e) {
      $("#msgErr").textContent = e?.data?.message || "Пользователь не найден или ошибка отправки.";
    }
  });
}
