  ...
  fetchAiStatus().then((st) => {
    if (st.configured) {
      ...
      appendAiMessage(
        messagesEl,
        "system",
        "ИИ не подключён. Администратору нужно добавить GIGACHAT_CLIENT_ID и GIGACHAT_SECRET_KEY в .env (см. .env.example)."
      );
      ...
      <div class="muted" style="margin-bottom:14px">
        ${
          st.configured
            ...
            : "ИИ не настроен: добавьте GIGACHAT_CLIENT_ID и GIGACHAT_SECRET_KEY в .env (см. .env.example)."
        }
      </div>
...
async function bindCreateAiButtons() {
  ...
  if (!st.configured) {
    [improveBtn, titleBtn].filter(Boolean).forEach((b) => {
      b.disabled = true;
      b.title = "Настройте GigaChat в .env";
    });
    return;
  }
...
async function bindPostAiComment(postId) {
  ...
  if (!st.configured) {
    btn.disabled = true;
    btn.title = "Настройте GigaChat в .env";
    return;
  }
